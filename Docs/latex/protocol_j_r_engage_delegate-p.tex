...
Sent if the authentication was canceled for any reason other than an error. For example, the user hits the \char`\"{}Cancel\char`\"{} button, any class (e.g., the \hyperlink{class_j_r_engage}{JREngage} delegate) calls the cancelAuthentication message, or if configuration of the library is taking more than about 16 seconds (rare) to download. \hypertarget{protocol_j_r_engage_delegate-p_a6ac183348ebc0e858e8aad0682b5e1cd}{
...
\begin{Desc}
\item[\hyperlink{deprecated__deprecated000019}{Deprecated}]Please use \hyperlink{protocol_j_r_engage_delegate-p_a2cbed662f784706d2db038e1cb9ba3fd}{jrAuthenticationDidReachTokenUrl:withResponse:andPayload:forProvider:} instead. \end{Desc}
